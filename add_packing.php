...
<h2 style="text-align: center;" class="font-weight-bold mb-1"><b> <i>ADD PACKING INFORMATION</i></b></h2><br>
</div>
   <div class="row" >
    <div class="col-md-5">
          ...
          <optgroup >
              <?php
               ...
               while ($row4=mysqli_fetch_array($db4)) {?>
                 <option value="<?php echo $row4['id']; ?>"> <?php echo $row4['formula_name']; ?> </option>
               <?php }
              ?> 
          </optgroup> 
      </select>
...
formula_grand.formula_name AS formu , groups.title

FROM packings

INNER JOIN  formula_grand ON  packings.formula_id=formula_grand.id
