...
<h2 style="text-align: center;" class="font-weight-bold mb-1"><b> <i>RAW MATERIALS PURCHASE </i></b></h2><br>
</div>
                ...
                <div class="panel "  style="margin-left:3%">
                    <div class="panel-heading">
                        <h3 class="panel-title">
                            <i class="fa fa-fw ti-pencil"></i> Add Purchase
                        </h3>
...
for ($i=0; $i<$count;$i++) { 
(float)$qnty[$i]=$_POST['bonous'][$i]+$_POST['quantity'][$i];
  
$query="INSERT INTO `raw_material_purchase`( `product_id`, `packing_id`, `supplier_id`, `pur_price`, `dis_percent`, `dis_value`, `bonus`, `saletax`, `min_stock_level`, `max_stock_level`, `quantity`, `qnty_loss`, `total_price`, `pur_date`, `bill_no`, `bill_date`, `transport`)

VALUES( '".$_POST['product'][$i]."','".$_POST['packing'][$i]."','".$_POST['supplier'][$i]."',
'".(float)$_POST['prc'][$i]."','".(float)$_POST['discount'][$i]."','".(float)$_POST['cpdisc'][$i]."',  
'".$_POST['bonous'][$i]."','".(float)$_POST['saletax'][$i]."','".(float)$_POST['minstock'][$i]."',
