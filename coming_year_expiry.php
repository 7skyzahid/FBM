...
<h2 style="text-align: center;" class="font-weight-bold mb-1"><b> <i>Previous Month Expiry Report</i></b></h2><br>
</div>
                ...
                <div class="table-responsive">
                  <table class="table table-striped table-bordered table-hover" id="sample_1">
                    ...
                    <tbody id="display">
                      <?php
                      $serial = 0;
                      // $select = "SELECT stock_items.product_id ,stock_items.expiry_date,  stock_items.quantity, stock_items.pur_price, stock_items.total_price,stock_items.batch,stock_items.supplier_id, stock_items.date FROM stock_items WHERE YEAR(stock_items.expiry_date) = YEAR(DATE_SUB(CURDATE(), INTERVAL 1 YEAR))";
                      $select = "SELECT stock_items.product_id ,stock_items.expiry_date,  stock_items.quantity, stock_items.pur_price, stock_items.total_price,stock_items.batch,stock_items.supplier_id, stock_items.date FROM stock_items WHERE YEAR(stock_items.expiry_date) = YEAR(DATE_SUB(CURDATE(), INTERVAL 1 YEAR))";
